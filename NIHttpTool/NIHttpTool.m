...
+(BOOL)requestBeforeCheckNetWorkWithFailureBlock:(failureBlocks)errorBlock{
    ...
    if(!isFi){//无网络
        ...
        [NIHttpStatus startNetworkActivity];
    }
...
#pragma mark - 网络请求带缓存
+ (void)requestHttpWithCacheType:(HttpCacheType)cacheType requestType:(HttpRequestType)requestType expired:(HttpCacheExpiredTimeType)expiredType url:(NSString *)url params:(NSDictionary *)params success:(successBlock)success failure:(void (^)(NSError *error))failure
{
...
+ (void)postWithURL:(NSString *)url params:(NSDictionary *)params formDataArray:(NSArray *)formDataArray success:(successBlock)success failure:(void (^)(NSError *))failure
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [[self getHttpSessionManager] POST:urlStr parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull totalFormData) {
            for (NIFileDataModel *fileData in formDataArray) {
                [totalFormData appendPartWithFileData:fileData.data name:fileData.name fileName:fileData.filename mimeType:fileData.mimeType];
            }
...
+ (AFHTTPSessionManager *)getHttpSessionManager{
    ...
    manager.requestSerializer = [AFJSONRequestSerializer serializer];//@"application/octet-stream" 视频流
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/x-www-form-urlencoded",@"application/json", @"text/html",@"image/jpg",@"image/jpeg",@"image/png",@"application/octet-stream",@"application/x-www-form-urlencoded",@"text/json",@"text/javascript",@"text/plain", nil];
    return manager;
}
...
+(NSString *)getCurrentDate
{
    ...
    NSString * currentDate = [NSString stringWithFormat:@"%ld-%.2ld-%.2ld %.2ld:%.2ld:%.2ld",(long)year,(long)month,(long)day,(long)hour,(long)minute,(long)second];
