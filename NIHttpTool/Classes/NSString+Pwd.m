...
/**
 蚂蚁腿腿 StringToArray_nixs 2017年04月16日16:31:28
...
+(NSArray*)StringToArray_NI:(NSString*)targetStr{
    ...
    NSString *tempTail = [targetStr substringWithRange:NSMakeRange(targetStr.length-1, 1)];//求目的字符串最后一个字符
    NSString *tempHead = [targetStr substringWithRange:NSMakeRange(0, targetStr.length-1)];//去除字符串末尾的逗号
    if([tempTail isEqualToString:@","]){
        if ([tempHead length]>0) {//判断去除最后一个逗号后的字符串长度
            if ([tempHead isCommaExists]) {
                array = [tempHead componentsSeparatedByString:@","];//字符串按照,分隔成数组
            }else{
                array=@[tempHead];//后台给的数据是一个字符串+最后一个逗号的形式
            }
        }else{
            array = nil;
        }
...
+(NSString *)dateStringFromNumberTimer_NI:(NSString *)timerStr{
    ...
    //    NSTimeInterval time=[timerStr doubleValue];
    //    //计算距离1970年的NSDate
